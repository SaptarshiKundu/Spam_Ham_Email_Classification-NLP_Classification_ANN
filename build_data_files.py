...
for entry in enron_list:
    ...
    r = requests.get(url)
    path = "raw_data/" + entry + ".tar.gz"

    with open(path, 'wb') as f:
        f.write(r.content)
    print('...done! Archive saved to: ' + path)

    # Unpack current archive this will unpack to /raw_data/enron1, etc
    print("Unpacking contents of " + entry + " archive...")
    shutil.unpack_archive(path, "raw_data/")
    print("...done! Archive unpacked to: raw_data/" + entry)
...
for directory in enron_list:
    ...
    ham_folder = "raw_data/" + directory + "/ham"
    spam_folder = "raw_data/" + directory + "/spam"
    i = 0
...
# Writing to CSV
mails.to_csv("data/spam_ham_enron_dataset.csv", index_label="Message_ID", escapechar='\\')
